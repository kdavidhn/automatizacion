	...
	case 'denegar':
      ...
      $valor = "select no_solicitud from tbl_voae_actividades where id_actividad_voae = '$id_actividad_voae'";
	    ...
	    $bt_no_solicitud = $result_valor->fetch_array(MYSQLI_ASSOC);
 		...
 		bitacora::evento_bitacora($Id_objeto, $_SESSION['id_usuario'], 'DENEGO', 'EL No DE SOLICITUD"' . $bt_no_solicitud['no_solicitud'] . '"');
	break;
		...
		$rspta=$actividad->listar1();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
 				...
 				'<button id="btn_denegar"title="Denegar Solicitud " class="btn btn-danger pull-right"  '.$_SESSION["btn_denegar"].' onclick="denegar('.$reg->id_actividad_voae.')"><i class="fa fa-arrow-circle-left"></i></button>',
 				"1"=>$reg->no_solicitud,
 				"2"=>$reg->fch_solicitud,
